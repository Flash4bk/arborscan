...
app = FastAPI(title="ArborScan API", description="AI-анализ деревьев", version="1.0")
...
async def analyze_tree(file: UploadFile = File(...), lat: float = 55.75, lon: float = 37.62):
    """
    Основной эндпоинт для анализа дерева по фотографии.
    Возвращает: вид, параметры, погоду, почву, риск.
    """
    try:
